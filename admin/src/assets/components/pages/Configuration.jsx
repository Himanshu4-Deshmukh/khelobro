        ...
        >
          {configs &&
            Object.entries(configs).map(([key, value]) => {
              ...
              // Deposit payment method visibility toggles
              if (
                key === "SHOW_PAYTM_DEPOSIT" ||
                key === "SHOW_QR_DEPOSIT" ||
                key === "SHOW_UPI_DEPOSIT"
              ) {
                const labelMap = {
                  SHOW_PAYTM_DEPOSIT: "Pay by Paytm App",
                  SHOW_QR_DEPOSIT: "Pay by QR Code",
                  SHOW_UPI_DEPOSIT: "Pay on UPI ID",
                };
                return (
                  <div key={key} className="my-3">
                    <div className="small fw-bold text-dark">
                      Deposit Option — {labelMap[key]}
                    </div>
                    <select
                      name={key}
                      className="form-control"
                      defaultValue={value === false || value === 0 || value === "false" ? 0 : 1}
                    >
                      <option value={1}>Visible (ON)</option>
                      <option value={0}>Hidden (OFF)</option>
                    </select>
                  </div>
                );
              }
