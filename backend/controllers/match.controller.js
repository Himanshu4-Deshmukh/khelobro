...
// // ====== NEW: Track timeouts for unjoined matches ======
// const pendingMatchTimeouts = new Map(); // matchId → timeoutId
...
//     // ====== AUTO-CANCEL IF NO JOINER IN 3 MINUTES ======
//     const timeoutId = setTimeout(async () => {
//       try {
//         const stillOpen = await ManualMatch.findById(m._id);
//         if (stillOpen && stillOpen.status === "open") {
//           await ManualMatch.deleteOne({ _id: m._id });
//           await _log({
//             matchId: m._id,
//             message: `Match auto-cancelled after 3 minutes (no joiner). Created by ${req.user.fullName} (${req.user.mobileNumber})`,
//           });
//         }
//       } catch (err) {
//         console.error("Auto-cancel match error:", err);
//       } finally {
//         pendingMatchTimeouts.delete(m._id.toString());
//       }
//     }, 2 * 60 * 1000); // 2 minutes

//     pendingMatchTimeouts.set(m._id.toString(), timeoutId);
//     // ===================================================

//     await _log({
//       matchId: m._id,
//       message: req.user.fullName + "(" + req.user.mobileNumber + ") created match",
//     });
...
const runningMatchTimeouts = new Map(); // matchId → timeoutId
...
export const createMatch = async (req, res) => {
  try {
    ...
    // ====== AUTO-DELETE IF NO JOINER IN 2 MINUTES ======
    const timeoutId = setTimeout(async () => {
      try {
        const stillOpen = await ManualMatch.findById(m._id);
        if (stillOpen && stillOpen.status === "open") {
          ...
          await _log({
            matchId: m._id,
            message: `Match auto-deleted after 2 minutes (no joiner). Created by ${req.user.fullName} (${req.user.mobileNumber})`,
          });
        }
      } catch (err) {
        console.error("Auto-delete unjoined match error:", err);
      } finally {
        openMatchTimeouts.delete(m._id.toString());
      }
    }, 2 * 60 * 1000); // 2 minutes

    openMatchTimeouts.set(m._id.toString(), timeoutId);
    // ===================================================
...
// export const cancelMatchAndRefund = async (matchId) => {
//   try {
//     const match = await ManualMatch.findById(matchId);

//     if (!match) {
//       console.log(`Match ${matchId} not found for auto-cancellation`);
//       return;
//     }

//     // Only cancel if match is still in "running" status and no room code has been set
//     if (match.status !== "running" || match.roomCode) {
//       console.log(`Match ${matchId} already processed or has room code`);
//       return;
//     }
...
//         $set: {
//           status: "cancelled",
//           cancelReason: "No room code entered within time limit"
//         }
//       }
//     );
...
//         reward: refundAmount,
//         bonus: 0,
//         remark: "Match Cancelled - Auto Refund",
//         status: "completed",
//         txnType: "credit",
//         txnCtg: "refund",
//         matchId: match._id,
...
//     await _log({
//       matchId: match._id,
//       message: `Match auto-cancelled after 2.5 minutes (no room code). Refunded ₹${refundAmount} to both players.`
//     });

//     console.log(`Match ${matchId} auto-cancelled and refunded successfully`);
//   } catch (error) {
//     console.error(`Error in cancelMatchAndRefund for match ${matchId}:`, error);
//     await _log({
//       matchId: matchId,
//       message: `Error in auto-cancel: ${error.message}`
//     });
//   }
// };
...
//       status: "running",
//       $or: [
//         { "host.userId": joiner._id, "host.result": null },
//         { "joiner.userId": joiner._id, "joiner.result": null },
//       ],
//     }).sort({ createdAt: -1 });
//     const runningmatch2 = await OnlineGame.findOne({
//       $and: [{ status: "running" }, { $or: [{ "blue.userId": joiner._id }, { "green.userId": joiner._id }] }],
//     }).sort({ createdAt: -1 });
...
//       { _id: match._id, status: "open", "host.userId": host._id },
//       {
//         $set: {
//           status: "running",
...
//     // ====== CLEAR AUTO-CANCEL TIMEOUT SINCE SOMEONE JOINED ======
//     const timeoutId = pendingMatchTimeouts.get(match._id.toString());
//     if (timeoutId) {
//       clearTimeout(timeoutId);
//       pendingMatchTimeouts.delete(match._id.toString());
//     }
//     // ============================================================
...
//     await ManualMatch.updateOne(
//       { _id: { $ne: match._id }, status: "open" },
//       { $pull: { joinerReqs: { userId: joiner._id } } }
//     );
//     await ManualMatch.updateOne(
//       { _id: { $ne: match._id }, status: "open" },
//       { $pull: { joinerReqs: { userId: host._id } } }
//     );
...
//     // ====== CLEAR THE "NO JOINER" TIMEOUT SINCE SOMEONE JOINED ======
//     const openTimeout = openMatchTimeouts.get(match._id.toString());
//     if (openTimeout) {
//       clearTimeout(openTimeout);
//       openMatchTimeouts.delete(match._id.toString());
//     }
//     // ================================================================
...
//     // ====== KEEP EXISTING LOGIC: CANCEL AFTER 2.5 MIN IF NO ROOM CODE ======
//     setTimeout(() => {
//       cancelMatchAndRefund(match._id);
//     }, 1000 * 150); // 2.5 minutes = 150 seconds
...
// Add this at the top of your file with other Maps
...
export const joinMatch = async (req, res) => {
  try {
    ...
    // ====== AUTO-CANCEL AFTER 2.5 MIN IF NO ROOM CODE ENTERED ======
    const runningTimeoutId = setTimeout(async () => {
      try {
        await cancelMatchAndRefund(match._id);
      } catch (error) {
        console.error("Error in auto-cancel timeout:", error);
      } finally {
        runningMatchTimeouts.delete(match._id.toString());
      }
    }, 1000 * 150); // 2.5 minutes = 150 seconds

    runningMatchTimeouts.set(match._id.toString(), runningTimeoutId);
    // ================================================================
...
// ====== CANCEL MATCH AND REFUND FUNCTION ======
export const cancelMatchAndRefund = async (matchId) => {
  try {
    const match = await ManualMatch.findById(matchId);
    ...
    if (match.host.txnId) {
      ...
      if (hostTxn) {
        await Transaction.create({
          ...
          amount: refundAmount,
          cash: hostTxn.cash || 0,
          reward: hostTxn.reward || 0,
          bonus: hostTxn.bonus || 0,
          remark: "Match Cancelled - Auto Refund (No Room Code)",
          status: "completed",
          txnType: "credit",
          txnCtg: "refund",
          matchId: match._id,
    ...
    if (match.joiner.txnId) {
      ...
      if (joinerTxn) {
        await Transaction.create({
          ...
          userId: match.joiner.userId,
          amount: refundAmount,
          cash: joinerTxn.cash || 0,
          reward: joinerTxn.reward || 0,
          bonus: joinerTxn.bonus || 0,
          remark: "Match Cancelled - Auto Refund (No Room Code)",
          status: "completed",
          txnType: "credit",
          txnCtg: "refund",
    ...
    await _log({
      matchId: match._id,
      message: `Match auto-cancelled after 2.5 minutes (no room code entered). Refunded ₹${refundAmount} to both players.`
    });
